...
class ImageSlicer(object):
    ...
    def __read_images(self):
        ...
        for im in image_names:
            image = imageio.imread(os.path.join(dir_path,im))
            print (image)
            Images.append(image)
        return Images
    ...
    def __convolution_op(self, Image):
        ...
        for i in range(n_rows-1):
            for j in range(n_columns-1):
                ...
                if (new_start_x+self.size[0] >= Image.shape[0]):
                    new_start_x -= new_start_x+self.size[0] - Image.shape[0]
                if (new_start_y+self.size[1] >= Image.shape[1]):
                    new_start_y -= new_start_y+self.size[1] - Image.shape[1]
                try:
    ...
    def save_images(self,transformed, save_dir, prefix):
        if not(os.path.exists(save_dir)):
            ...
            for key, val in transformed.items():
                ...
                for i, j in enumerate(val):
                    if j.shape[:2] != self.size:
                        print (j.shape[:2], self.size)
                        continue
                    imageio.imsave(os.path.join(path, prefix + "_" + str(i+1)+'.png'), j)
