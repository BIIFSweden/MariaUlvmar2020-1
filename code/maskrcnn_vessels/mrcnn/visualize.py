...
def save_instances(image, boxes, masks, class_ids, class_names,
                      ...
                      score_threshold=0.4, outputFile=None, keep=None):
    """
    ...
    if not colors:
        ...
        colors = [color[class_] for class_ in class_ids]
    ...
    for i in range(N):
        ...
        x_coor = list(verts[:,0].astype(np.int32))
        y_coor = list(verts[:,1].astype(np.int32))
        for x1, y1, x2, y2 in zip(x_coor, y_coor, x_coor[1:]+[x_coor[0]], y_coor[1:]+[y_coor[0]]):
            ...
            for step in range(int(nb_steps)+1):
                ...
                try:
                    masked_image[y, x] = color
                except Exception as e:
                    print (e) 
                    pass
    imageio.imsave(outputFile, masked_image.astype(np.uint8))
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            ax.add_patch(p)
    ax.imshow(masked_image.astype(np.uint8)*3)
    if auto_show:
        plt.show()
